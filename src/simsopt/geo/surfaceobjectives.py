...
__all__ = ['Area', 'Volume', 'ToroidalFlux', 'PrincipalCurvature',
           ...
           'AspectRatio', 'SurfaceSurfaceDistance']
...
class SurfaceSurfaceDistance(Optimizable):
    r"""
    Computes a penalty for surface-to-surface proximity:

    .. math::
        J = \frac{1}{N_1 N_2} \sum_{i,j} \max(0, d_{\min} - \|\mathbf{s}_1^i - \mathbf{s}_2^j\|)^2

    where :math:`\mathbf{s}_1^i` and :math:`\mathbf{s}_2^j` are points on the two surfaces
    and :math:`d_{\min}` is the minimum allowed separation. The penalty is zero when all
    point pairs are separated by at least :math:`d_{\min}`.
    """
